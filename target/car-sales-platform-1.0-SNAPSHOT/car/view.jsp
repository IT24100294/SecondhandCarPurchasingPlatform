...
<div class="container">
    ...
    <% Car[] similarCars = (Car[]) request.getAttribute("similarCars"); %>
    <% Review[] recentReviews = (Review[]) request.getAttribute("recentReviews"); %>
    ...
    <div style="margin-top: 40px;">
        ...
        Review[] recentReviews = null;

        if (reviewServlet != null) {
            averageRating = reviewServlet.getAverageRatingForCar(car.getId());
            Review[] allReviews = reviewServlet.getReviewsForCar(car.getId());
            // Get up to 2 most recent reviews for preview
            int previewCount = Math.min(2, allReviews.length);
            recentReviews = new Review[previewCount];
            for (int i = 0; i < previewCount; i++) {
                recentReviews[i] = allReviews[i];
            }
        }
        %>
            ...
            <div style="display: flex; align-items: center; margin-bottom: 20px;">
                ...
                <div>
                    ...
                    <span style="margin-left: 5px; color: #666;">
                        (<%= recentReviews != null ? recentReviews.length : 0 %> <%= (recentReviews != null && recentReviews.length == 1) ? "review" : "reviews" %>)
                    </span>
                </div>
            </div>

            <% if (recentReviews == null || recentReviews.length == 0) { %>
    ...
    <% if (similarCars != null && similarCars.length > 0) { %>
        <div style="margin-top: 40px;">
