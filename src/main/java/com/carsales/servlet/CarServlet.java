...
public class CarServlet extends HttpServlet {
    ...
    private void listCars(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (sortType != null) {
            switch (sortType) {
                case "price_asc":
                    MergeSort.sort(carsArray, MergeSort.SortOrder.ASCENDING);
                    break;
                case "price_desc":
                    MergeSort.sort(carsArray, MergeSort.SortOrder.DESCENDING);
                    break;
                // Add other sorting cases here if needed
            }
        }
    ...
    private void addCar(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (ServletFileUpload.isMultipartContent(request)) {
            try {
                ...
                // Convert List<FileItem> to FileItem[]
                FileItem[] itemsArray = items.toArray(new FileItem[0]);
                String[] imageNames = FileUtil.saveUploadedFiles(itemsArray, savePath);
        ...
        if (car != null) {
            ...
            for (Car otherCar : allCars) {
                if (otherCar.getId() != car.getId() &&
                    ...
                    similarCount++;
                    if (similarCount >= 3) break; // Limit to 3 similar cars
                }
            }

            // Create array of exact size
            Car[] similarCars = new Car[similarCount];
            int index = 0;
            ...
            for (Car otherCar : allCars) {
                if (otherCar.getId() != car.getId() &&
                        (otherCar.getMake().equals(car.getMake()) ||
                                Math.abs(otherCar.getPrice() - car.getPrice()) <= priceRange)) {
                    similarCars[index++] = otherCar;
                    if (index >= 3) break; // Limit to 3 similar cars
                }
            }
